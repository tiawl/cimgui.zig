...
// $ ./zig-out/bin/example_sdl3_zvulkan+zopengl3+sdlgpu3
// The command line to run it with zOpenGL3 backend:
// $ env RENDERER=zOpenGL3 ./zig-out/bin/example_sdl3_zvulkan+zopengl3+sdlgpu3
// The command line to run it with SDLGPU3 backend:
// $ env RENDERER=SDLGPU3 ./zig-out/bin/example_sdl3_zvulkan+zopengl3+sdlgpu3
...
pub fn main(init: std.process.Init) !void {
    ...
    switch (std.meta.stringToEnum(build.Renderer, init.environ_map.get("RENDERER") orelse @tagName(default_renderer)) orelse return error.UnknownRendererBackend) {
        inline default_renderer, .zOpenGL3, .SDLGPU3 => |renderer| {
