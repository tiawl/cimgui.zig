...
const c = @import("c");
pub const glfw = @import("common/glfw");
...
pub fn loadFunctions(api_version: u32) !void {
    if (!c.cImGui_ImplVulkan_LoadFunctions(api_version, loader)) return error.ImGuiVulkanLoad;
}
...
pub fn createWindowSurface(instance: *c.VkInstance, surface: *c.VkSurfaceKHR) c.VkResult {
    return c.glfwCreateWindowSurface(instance.*, glfw.window, null, surface);
}
...
pub fn initForVulkan() !void {
    if (!c.cImGui_ImplGlfw_InitForVulkan(glfw.window, true)) return error.cImGui_ImplGlfw_InitForVulkan;
}
