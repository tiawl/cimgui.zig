...
fn SetupVulkan(allocator: std.mem.Allocator, instance_extensions: *std.ArrayList([*:0]const u8)) !void {
    ...
    app_info.pApplicationName = "example_sdl3_vulkan";
...
pub fn main() !void {
    ...
    // Setup SDL
    if (!c.SDL_Init(c.SDL_INIT_VIDEO | c.SDL_INIT_GAMEPAD)) return error.SDL_initFailure;
    defer c.SDL_Quit();
    ...
    var w: i32 = undefined;
    var h: i32 = undefined;
    if (!c.SDL_GetWindowSize(window, &w, &h)) return error.SDL_GetWindowSizeFailure;
    var wd = &g_MainWindowData;
    try SetupVulkanWindow(wd, surface, w, h);
    defer CleanupVulkanWindow();
    if (!c.SDL_SetWindowPosition(window, c.SDL_WINDOWPOS_CENTERED, c.SDL_WINDOWPOS_CENTERED)) return error.SDL_SetWindowPositionFailure;
    if (!c.SDL_ShowWindow(window)) return error.SDL_ShowWindowFailure;
    ...
    if (!c.cImGui_ImplSDL3_InitForVulkan(window)) return error.cImGui_ImplSDL3_InitForVulkanFailure;
